...
async def validate_token(session):
    ...
    This function implements a token validation strategy with the following features:
    - Returns a cached token if it's still valid (with a 30-second buffer before expiration)
    - Uses async locking to prevent race conditions when multiple coroutines request tokens simultaneously
    - Performs a double-check pattern after acquiring the lock to ensure thread safety
    - Fetches a new token from the Platzi API if the cached token is expired or missing
...
async def get_token():
    ...
    removes the Accept header to prevent 406 errors, and validates the token before
    returning.
    ...
    async with aiohttp.ClientSession(
        cookies=config.COOKIES, headers=headers
    ) as session:
